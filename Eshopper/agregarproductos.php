<?php 
  //Crea una variable vacía llamada datos para guardar en ella
   const GUARDAR = 'Guardar';
   const VER_DATOS = 'Ver datos';
   $datos = [];
  //Crea variables en los que se guarda el valor que tienen en la base de datos
   $ID = $_POST['ID'] ?? null;
   ...
   if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['operacion'])){
     ...
     if(file_exists('archivo.txt')){
      ...
      //Se saca el contenidp del archivo(base de datos), y lo convierten en string si el archivo existe
         foreach($lineas as $linea){
            list($ID, $productoE, $precioE, $categoriaE) = explode(',', $linea);
            $datos[] = ['ID' => $ID, 'producto' => $productoE, 'precio' => $precioE, 'categoria' => $categoriaE];
           //En este ciclo, a las variables se les da el valor string que tienen en la base de datos
         }
     }
   }
   }
...
'; //Recorre con un ciclo cada elemento/datos del archivo que conecta con la base de datos
}
$body .= '</table>';
}
$body .= '
